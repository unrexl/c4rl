...
export function persistentState<T extends object>(name: string, selector: (state: RootState) => T, defaultValue: T): T {
	try {
		const serializedState = read(`_c4rl/${name}.json`);

		if (serializedState === undefined) {
			write(`_c4rl/${name}.json`, HttpService.JSONEncode(defaultValue));
			return defaultValue;
		}
...
async function autosave(name: string, selector: (state: RootState) => object) {
	...
	function save() {
		...
		write(`_c4rl/${name}.json`, HttpService.JSONEncode(state));
	}
